...
from nilm_models.ss2s.shortseq2pointdisaggregator import ShortSeq2PointDisaggregator
...
ss2s = ShortSeq2PointDisaggregator(window_size=100)

if not Path("model-redd5.h5").exists():
    ss2s.train(train_mains, train_meter, epochs=5, sample_period=1)
    ss2s.export_model("model-redd5.h5")
else:
    ss2s.import_model("model-redd5.h5")
...
if not Path("disag-out.h5").exists():
    ...
    ss2s.disaggregate(test_mains, output, train_meter, sample_period=1)
    output.close()
